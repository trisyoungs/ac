...
bool Command::function_OperatorDivide(CommandNode *c, ReturnValue &rv)
{
	...
	else switch (id)
	{
		case (VTypes::IntAIntA):
			for (int i=0; i<lhs.arraySize(); ++i) lhs.setElement(i, lhs.asInteger(i,b) / rhs.asInteger(i,b)); rv = lhs;
			break;
		case (VTypes::IntADblA):
		case (VTypes::DblAIntA):
		case (VTypes::DblADblA):
			for (int i=0; i<lhs.arraySize(); ++i) lhs.setElement(i, lhs.asDouble(i,b) / rhs.asDouble(i,b)); rv = lhs;
			break;
		case (VTypes::IntAInt):
			for (int i=0; i<lhs.arraySize(); ++i) lhs.setElement(i, lhs.asInteger(i,b) / rhs.asInteger(b)); rv = lhs;
			break;
		case (VTypes::IntADbl):
			for (int i=0; i<lhs.arraySize(); ++i) lhs.setElement(i, (signed long int) (lhs.asInteger(i,b) / rhs.asDouble(b))); rv = lhs;
			break;
		case (VTypes::DblAInt):
		case (VTypes::DblADbl):
			for (int i=0; i<lhs.arraySize(); ++i) lhs.setElement(i, lhs.asDouble(i,b) / rhs.asDouble(b)); rv = lhs;
			break;
		case (VTypes::IntInt):
		...
		case (VTypes::VecInt):
		case (VTypes::VecDbl):
			rv.set(lhs.asVector(b) / rhs.asDouble(b)); 
			break;
		case (VTypes::VecVec):
			rv.set(lhs.asVector(b) / rhs.asVector(b));
			break;
		case (VTypes::IntAVec):
		case (VTypes::DblAVec):
			...
			{
				Vec3<double> v(lhs.asDouble(0,b), lhs.asDouble(1,b), lhs.asDouble(2,b));
				rv.set(v / rhs.asVector());
			}
			break;
		case (VTypes::VecIntA):
		case (VTypes::VecDblA):
			...
			{
				Vec3<double> v(rhs.asDouble(0,b), rhs.asDouble(1,b), rhs.asDouble(2,b));
				rv.set(v / lhs.asVector());
			}
			break;
		default:
			printf("The operator '/' cannot act between %s and %s.\n", VTypes::aDataType(rv.type(),rv.arraySize()), VTypes::aDataType(rhs.type(),rhs.arraySize()));
